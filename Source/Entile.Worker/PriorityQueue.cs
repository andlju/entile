...
{
    // This class was written by Alexey Kurakin
    ...
    public class PriorityQueue<TPriority, TValue>
    {
        ...
        private void HeapifyFromBeginningToEnd(int pos)
        {
            ...
            while (true)
            {
                ...
                int left = 2 * pos + 1;
                int right = 2 * pos + 2;
                if (left < _baseHeap.Count && _comparer.Compare
                                                  (_baseHeap[smallest].Key, _baseHeap[left].Key) > 0)
                    smallest = left;
                if (right < _baseHeap.Count && _comparer.Compare
                                                   (_baseHeap[smallest].Key, _baseHeap[right].Key) > 0)
                    smallest = right;
